...
def get_available_backgrounds():
    ...
    if not os.path.exists(backgrounds_dir):
        click.echo(f"⚠️  No backgrounds directory found at: {backgrounds_dir}")
        return []
    ...
    if not background_files:
        click.echo(f"⚠️  No PNG files found in: {backgrounds_dir}")
        return []
    
    click.echo(f"📎 Found {len(background_files)} background images")
...
def copy_background_for_document(background_files, output_dir, doc_number):
    ...
    try:
        ...
        click.echo(f"❌ Failed to copy background for document {doc_number}: {e}")
        return None, None
...
def generate_documents(entity_file, document_type, language, output_dir, start_index):
  ...
  * {{
    font-family: Arial, 'Segoe UI', 'DejaVu Sans', 'Helvetica Neue', Helvetica, sans-serif !important;
        ...
        click.echo(f"✅ Successfully generated {len(entity_data_list)} documents!")
        click.echo(f"📁 HTML files saved to: {os.path.abspath(output_dir)}")
        click.echo(f"📄 Document metadata saved to: {os.path.abspath(metadata_filepath)}")
