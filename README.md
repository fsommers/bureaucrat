...
./generate \
 ...
 -e "patient sex" \
 -e "medical insurance company name" \
 -e "insurance policy number" \
 -e "list of prexisting conditions" \
 -e "list of allergies"
```
...
`-e` lets you list the data items you want to include in the documents. You can specify multiple `-e` parameters (as shown above) or use comma-separated values in a single `-e` parameter. Synthetic values for those items will be generated by an LLM.
...
./generate \
  ...
  -l de \
  -e "Vermieter Name" \
  -e "Vermieter Addresse" \
  -e "Mieter Name" \
  -e "Mieter Addresse" \
  -e "Mieter Geburtsdatum" \
  -e "Miete pro Monat" \
  -e "Beginn des Mietverhältnisses" \
...
./generate \
 ...
 -e "tour company address" \
 -e "tour company phone number" \
 -e "tour company email" \
 -e "American customer name" \
 -e "American customer email" \
 -e "American customer phone" \
 -e "tour dates" \
 -e "tour itinerary" \
 -e "list of accommodations in the tour"
```
...
<table>
<tr>
<td><img src="assets/medical1.png" alt="Medical Invoice 1" width="450"></td>
</tr>
</table>
...
```bash
# Basic usage - generates PDFs directly (comma-separated format)
./generate -t "business invoice" -e "customer name,amount,date" -c 10

# Same command using multiple -e parameters
./generate -t "business invoice" -e "customer name" -e "amount" -e "date" -c 10

# Generate documents in Spanish (multiple -e format)
./generate -t "contrato de servicios" -e "cliente" -e "monto" -e "fecha" -c 20 -l es -o spanish_docs
...
- `-e, --entity-fields`: Entity fields - can be specified multiple times or comma-separated (required)
...
```bash
# Basic entity generation for catering invoices (comma-separated format)
python generate_entities.py -t "catering invoice for birthday party" -e "customer name,invoice number,total amount" -c 100

# Same command using multiple -e parameters
python generate_entities.py -t "catering invoice for birthday party" -e "customer name" -e "invoice number" -e "total amount" -c 100
...
- `-e, --entity-fields TEXT`: Entity fields - can be specified multiple times (e.g., `-e "name" -e "date"`) or as comma-separated values (e.g., `-e "name,date"`) (required)
...
- `-e, --entity-fields TEXT`: Entity fields - can be specified multiple times or comma-separated (required for manual mode)
...
```bash
# Generate 10 invoices with PDFs in one command (comma-separated)
./generate -t "business invoice" -e "customer name,invoice number,amount,date" -c 10

# Or using multiple -e parameters
./generate -t "business invoice" -e "customer name" -e "invoice number" -e "amount" -e "date" -c 10
