# Bureaucrat: Synthetic Business Documents at Scale
...
For example, this command will result in 10 medical intake forms being generated, with each form having possibly different layouts and different data.

`-t` allows you to specify the document type, and
`-e` let's you list the data items you want to include in the documents. The values for those items will be generated by an LLM:
...
You can do the same in other languages. For example, to generate 5 unique German apartment rental agreements:

```
./generate \
  -t "Wohnungsmietvertrag" \ 
  -e "Vermieter Name, Vermieter Addresse, Mieter Name, Mieter Addresse, Mieter Geburtsdatum, Miete pro Monat, Beginn des Mietverhältnisses" \
  -c 5 \
  -l de \
...
The data is region-appropriate, such as using German(-sounding) names, German addresses, currency formatting, phone numbers, and emails:
...
Other character sets are also supported. This is an invoice for a custom 1-week tour of Thailand:

```
./generate -t "An invoice for a 1-week custom tour of Thailand" \
-e "Thai tour company name, tour company address, tour company phone number, tour company email, American customer name, American customer email, American customer phone, tour dates, tour itinerary, list of accommodations in the tour" \
 -c 1 \
 -l th \
