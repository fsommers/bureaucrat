...
For example, the following command results in 10 medical intake forms PDFs, with each form having different layouts and different data:
...
`-t` allows you to specify what to generate,
...
The result is 3 PDF documents for a medical office intake form:
...
The last document illustrates the ability to render document backgrounds that mimic scanned or photographed paper documents. (In this case, perhaps the office was a bit hasty while performing tests...)
...
You can do the same in other languages. The following generates 5 unique German apartment rental agreements:
...
`-I` parameter specify optional instructions to the LLM -- in this case, to include legal language appropriate for a German apartment rental agreement. 

The resulting data is region-appropriate, such as using German(-sounding) names, German addresses, currency formatting, phone numbers, and emails:
